...
- **Fixed: macintosh + cheese icons looked too far apart.** Measured their
  actual pixel content and found 19-25% built-in blank margin per side in
  their 128x128 canvas, vs. ~9.4% (3/32 grid cells) for almost every other
  icon in the set - so even at zero `<td>` padding, this specific pair
  looked much farther apart than e.g. fish+salmon. Fixed both halves of
  it: `padding: 0 2px` → `padding: 0` on the icon `<td>`s, and regenerated
  both icons in `generate-icons.mjs` (and the Python port used to actually
  produce the PNGs) with their artwork rescaled to the ~9.4% convention
  (all other 17 icons regenerated byte-identical, confirmed via `git
  status`, and all 19 stayed pixel-distinct from each other).
- **Fixed: icons still ~3x-icon-width apart after the two fixes above.**
  The inner icon `<table>` relied on `display: inline-table` to size
  itself to its content (so `margin: 0 auto` could center it as a compact
  block). Once that table moved inside the outer table's `<td>` (the
  icon-alignment fix), `display: inline-table` apparently stopped being
  honored - the table stretched to fill its container, and its 2
  auto-sized `<td>`s each grabbed half that (much too generous) space,
  centering their own icon independently and pushing the two far apart.
  Fixed by computing an exact pixel width with Liquid math
  (`{{ icon_count | times: 44 }}px` - `times` is a standard Liquid
  filter, unlike the unsupported parenthesized `{% if %}` from the bug
  above) instead of relying on implicit sizing, giving each inner `<td>`
  an explicit `width: 44px`, and centering with `margin: 0 auto` against
  that now-definite width.
